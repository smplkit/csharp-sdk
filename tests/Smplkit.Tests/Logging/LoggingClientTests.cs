...
public class LoggingClientTests
{
    ...
    [Fact]
    public void BuildBulkItem_SetsLevelFromDiscoveredLogger()
    {
    ...
    public void BuildBulkItem_AllLevels_LevelMatchesWireString(LogLevel level, string expected)
    {
        var discovered = new Smplkit.Logging.Adapters.DiscoveredLogger("logger-name", level);

        var item = LoggingClient.BuildBulkItem(discovered);

        Assert.Equal(expected, item.Level);
    }
