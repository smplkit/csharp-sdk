...
/// (<c>"exponential"</c>), and the generated <c>retry_on.reasons</c> list carries
/// no element converter, so it serializes as integers. A single
/// <see cref="RetryPolicyJsonConverter"/> owns the whole attributes object and
/// emits / parses the exact wire shape (<c>backoff</c> as its lowercase slug,
/// <c>reasons</c> as their string values), matching the canonical SDKs.</para>
...
/// <c>retry_on</c>, and <c>max_delay_seconds</c> when set — leaving the
/// server-managed timestamps / version off the body. On read it parses the full
/// attributes object, tolerating <c>backoff</c> in any case and <c>reasons</c> as
/// their string values.
/// </remarks>
internal sealed class RetryPolicyJsonConverter : JsonConverter<RetryPolicy>
{
    public override RetryPolicy Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        var ro = el.GetProperty("retry_on");
        var statuses = new List<int>();
        foreach (var s in ro.GetProperty("statuses").EnumerateArray())
            statuses.Add(s.GetInt32());
        var reasons = new List<Reasons>();
        foreach (var r in ro.GetProperty("reasons").EnumerateArray())
            reasons.Add(Enum.Parse<Reasons>(r.GetString()!, ignoreCase: true));
        ...
        var policy = new RetryPolicy
        {
            ...
            Retry_on = new RetryOn { Statuses = statuses, Reasons = reasons },
        };
    ...
    public override void Write(Utf8JsonWriter writer, RetryPolicy value, JsonSerializerOptions options)
    {
        ...
        writer.WritePropertyName("retry_on");
        writer.WriteStartObject();
        writer.WritePropertyName("statuses");
        writer.WriteStartArray();
        foreach (var status in value.Retry_on.Statuses)
            writer.WriteNumberValue(status);
        writer.WriteEndArray();
        writer.WritePropertyName("reasons");
        writer.WriteStartArray();
        // Reason member names match their wire values exactly; emit them as strings
        // (the generated list has no element converter and would emit integers).
        foreach (var reason in value.Retry_on.Reasons)
